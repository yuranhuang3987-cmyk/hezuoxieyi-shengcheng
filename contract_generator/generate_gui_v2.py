...
class ContractGenerator:
    ...
    def setup_ui(self):
        ...
        # 文件选择区
        file_frame = LabelFrame(main_frame, text=" 选择申请表 ", 
        ...
        if DRAG_DROP_AVAILABLE:
            tip = Label(file_frame, text="提示: 直接拖动文件到此窗口 | 双击预览 | Delete删除", 
    ...
    def on_drop(self, event):
        ...
        # 状态栏
        self.status_var = StringVar(value="就绪 - 请添加申请表文件")
    ...
    def add_folder(self):
        ...
        if folder:
            files = [os.path.join(folder, f) for f in os.listdir(folder) if f.endswith('.docx')]
            self._add_files_to_list(files)
